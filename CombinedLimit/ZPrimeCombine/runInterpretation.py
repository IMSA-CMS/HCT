	...
	if args.CI or args.ADD:
		...
		for Lambda in Lambdas:
			for interference in config.interferences:
				...
				if len(config.channels) == 1 and not args.combineSigDY:
                                        if args.singlebin:
                                                if args.mass == -1:
                                                        args.mass = 2200
                                                cardName = cardDir + "/" + config.channels[0] + "_%d_%s_M%d"%(Lambda,interference,args.mass) + ".txt"
                                        else:
				...
				if not args.frequentist:
					if args.combineSigDY:
						subCommand = ["combine","-M","MarkovChainMC","%s"%cardName, "-n", "%s"%args.config , "-m","%d"%Lambda, "-i", "%d"%config.numInt, "--tries", "%d"%numToys ,  "--prior","Test","-v", "10"]
					else:
						subCommand = ["combine","-M","MarkovChainMC","%s"%cardName, "-n", "%s"%args.config , "-m","%d"%Lambda, "-i", "%d"%config.numInt, "--tries", "%d"%numToys ,  "--prior","Test","--rMax","%d"%getRange(Lambda,args.CI,args.usePhysicsModel),"-v","10"]
				else:
		...
		for massRange in masses:
			...
			while mass <= massRange[2]:
				...
				if not args.frequentist:
					subCommand = ["combine","-M","MarkovChainMC","%s"%cardName, "-n" "%s"%args.config , "-m","%d"%mass, "-i", "%d"%config.numInt, "-S", "%d"%args.systematics, "--tries", "%d"%numToys ,  "--prior","flat","--rMax","%d"%getRange(mass,args.CI,args.usePhysicsModel)]
				else:
	...
	if args.CI:
		...
		for mass in masses:
			for interference in config.interferences:
				...
				if not args.frequentist:
					subCommand = ["combine","-M","MarkovChainMC","%s"%cardName, "-n" "%s"%args.config , "-m","%d"%mass, "-i", "%d"%config.numInt, "-S", "%d"%args.systematics, "--tries", "%d"%numToys ,  "--prior","flat","--rMax","%d"%getRange(mass,args.DM,args.CI,args.usePhysicsModel)]
				else:
...
def submitToFNALCondor(args,config):
	...
	from prepareLPCSubmission import createRunCSH, createJDL
        ...
        os.chdir("CMSSW_8_1_0/src/submission/")
		...
		for Lambda in lambdas:
			for interference in config.interferences:
				...
				for channel in config.channels:
					for index,massBins in enumerate(config.binning):
						if index < len(config.binning) - 1:
							command.append("%s_%s=%s_%d_%s_M%s.txt"%(channel, massBins, channel,Lambda,interference, massBins))	
...
def summarizeConfig(config,args,cardDir):
	...
	if args.signif:
		...
		if args.expected:
			print "Calculating expected limits with %d toy datasets"%configexptToys
		else:
...
if __name__ == "__main__":
        ...
        parser.add_argument("-c", "--config", dest = "config", required=True, help="name of the configuration to use")
	...
	parser.add_argument("--nosystematics", dest = "systematics",action="store_const",const = 0, default = 1,help = "0 denotes no systematics and 1 denotes systematics, default is 1")
	...
	if (args.redo or args.write) and not args.LEE and not (args.CI or args.ADD):
		...
		if (args.int or args.combineSigDY) and args.expected:
