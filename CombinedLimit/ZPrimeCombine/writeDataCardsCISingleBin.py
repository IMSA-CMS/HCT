...
def main():
	...
	parser.add_argument("-s", "--signif", action="store_true", default=False, help="write card for significances")
        ...
        parser.add_argument("-a", action= "store_true", default = False, help="ADD")
	...
	for Lambda in config.lambdas:
		for interference in config.interferences:
                        name = "%s/%s_%d_%s_M%s" % (cardDir,args.chan, Lambda, interference, args.mass)
                        if args.inject or "toy" in tag:	
                            yields = createSingleBinCI(Lambda,interference, name,args.chan,args.config, args.mass ,args.combineSigDY,args.a,dataFile=injectedFile)
                        else:	
                            yields = createSingleBinCI(Lambda,interference, name,args.chan,args.config, args.mass, args.combineSigDY,args.a)
