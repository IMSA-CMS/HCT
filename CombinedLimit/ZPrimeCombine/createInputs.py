...
def createWS(massVal,minNrEv,name,channel,width,correlateMass,dataFile="",CB=True,write=True,useShapeUncert=False):
	...
	if CB:
		...
		if 'dielectron_Moriond2017' in channel and massVal > 2300:
			ws.factory("RooCBShape::cb_%s(mass_%s, mean[0.0], res_scaled%s, alpha_%s, n_%s)"%(channel,channel,peakName,channel,channel))
		else:	
...
def createHistogramsCI(L,interference,name,channel,scanConfigName,dataFile=""):
	...
	import pickle 
	if "muon" in channel:
		pkl = open("input/inputsCI/CIsignalYields_default.pkl", "r")
		signalYields_default = pickle.load(pkl)

		pkl = open("input/inputsCI/CIsignalYields_scaleDown.pkl", "r")
...
#		signalYields_pileup = pickle.load(pkl)
		...
		pkl = open("input/inputsCI/CIsignalYields_resolution.pkl", "r")
		signalYields_resolution = pickle.load(pkl)

		pkl = open("input/inputsCI/CIsignalYields_ID.pkl", "r")
		signalYields_ID = pickle.load(pkl)
	elif "electron" in channel:
		pkl = open("input/inputsCI/CIsignalYieldsEle_default.pkl", "r")
		signalYields_default = pickle.load(pkl)
		pkl = open("input/inputsCI/CIsignalYieldsEle_scaleUp.pkl", "r")
		signalYields_scaleUp = pickle.load(pkl)
		pkl = open("input/inputsCI/CIsignalYieldsEle_scaleDown.pkl", "r")
		signalYields_scaleDown = pickle.load(pkl)
		pkl = open("input/inputsCI/CIsignalYieldsEle_piledown.pkl", "r")
		signalYields_piledown = pickle.load(pkl)
		pkl = open("input/inputsCI/CIsignalYieldsEle_pileup.pkl", "r")
		signalYields_pileup = pickle.load(pkl)
	...
	for index, lower in enumerate(binning):	
		if index < len(binning)-1:
			...
			val = signalYields_default[label][int(lower)][0]
			err = signalYields_default[label][int(lower)][1]
			...
			if "electron" in channel:
				val = signalYields_scaleUp[label][int(lower)][0]
				sigHistScaleUp.SetBinContent(index+1,max(0,val))
				val = signalYields_piledown[label][int(lower)][0]
				sigHistPUDown.SetBinContent(index+1,max(0,val))
				val = signalYields_pileup[label][int(lower)][0]
...
def createHistogramsADD(L,interference,name,channel,scanConfigName,dataFile=""):
	...
	import pickle 
	#changed inputsADD to inputsCI Kaushal temp
	if "muon" in channel:
		pkl = open("input/inputsCI/ADDsignalYields_default.pkl", "r")
		signalYields_default = pickle.load(pkl)

		pkl = open("input/inputsCI/ADDsignalYields_scaleDown.pkl", "r")
		signalYields_scaleDown = pickle.load(pkl)
...
#		signalYields_pileup = pickle.load(pkl)
		...
		pkl = open("input/inputsCI/ADDsignalYields_resolution.pkl", "r")
		signalYields_resolution = pickle.load(pkl)

		pkl = open("input/inputsCI/ADDsignalYields_ID.pkl", "r")
		signalYields_ID = pickle.load(pkl)

	elif "electron" in channel:
		pkl = open("input/inputsCI/ADDsignalYieldsEle_default.pkl", "r")
		signalYields_default = pickle.load(pkl)
		pkl = open("input/inputsCI/ADDsignalYieldsEle_scaleUp.pkl", "r")
		signalYields_scaleUp = pickle.load(pkl)
		pkl = open("input/inputsCI/ADDsignalYieldsEle_scaleDown.pkl", "r")
		signalYields_scaleDown = pickle.load(pkl)
		pkl = open("input/inputsCI/ADDsignalYieldsEle_piledown.pkl", "r")
		signalYields_piledown = pickle.load(pkl)
		pkl = open("input/inputsCI/ADDsignalYieldsEle_pileup.pkl", "r")
		signalYields_pileup = pickle.load(pkl)
...
def createSingleBinCI(L,interference,name,channel,scanConfigName,mThresh,addSigDY,ADD,dataFile=""):
	...
	if "muon" in channel:
		pkl = open("input/inputsCI/"+fileBegin+"signalYields_default.pkl", "r")
		signalYields_default = pickle.load(pkl)
		pkl = open("input/inputsCI/"+fileBegin+"signalYields_scaleDown.pkl", "r")
		signalYields_scaleDown = pickle.load(pkl)
...
#		signalYields_pileup = pickle.load(pkl)
		...
		pkl = open("input/inputsCI/"+fileBegin+"signalYields_resolution.pkl", "r")
		signalYields_resolution = pickle.load(pkl)

		pkl = open("input/inputsCI/"+fileBegin+"signalYields_ID.pkl", "r")
		signalYields_ID = pickle.load(pkl)
	elif "electron" in channel:
		pkl = open("input/inputsCI/"+fileBegin+"signalYieldsEle_default.pkl", "r")
		signalYields_default = pickle.load(pkl)
		pkl = open("input/inputsCI/"+fileBegin+"signalYieldsEle_scaleUp.pkl", "r")
		signalYields_scaleUp = pickle.load(pkl)
		pkl = open("input/inputsCI/"+fileBegin+"signalYieldsEle_scaleDown.pkl", "r")
		signalYields_scaleDown = pickle.load(pkl)
		pkl = open("input/inputsCI/"+fileBegin+"signalYieldsEle_piledown.pkl", "r")
		signalYields_piledown = pickle.load(pkl)
		pkl = open("input/inputsCI/"+fileBegin+"signalYieldsEle_pileup.pkl", "r")
		signalYields_pileup = pickle.load(pkl)
	...
	if "muon" in channel:
		...
		if ADD:
			...
			label = "CITo2E_Lam%dTeV%s_%s"%(L,interference,channel)
	val = signalYields_default[label][int(mThresh)][0]
	err = signalYields_default[label][int(mThresh)][1]
	
	valScaleDown = signalYields_scaleDown[label][int(mThresh)][0]
	if "electron" in channel:
		valScaleUp = signalYields_scaleUp[label][int(mThresh)][0]
		valPUDown = signalYields_piledown[label][int(mThresh)][0]
		valPUUp = signalYields_pileup[label][int(mThresh)][0]

	if "muon" in channel:
		valID = signalYields_ID[label][int(mThresh)][0]
		valSmear = signalYields_resolution[label][int(mThresh)][0]
	...
	result["data"] = dataHist.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)	

 	err = ROOT.Double(0)	
	val = bkgHistDY.IntegralAndError(bkgHistDY.FindBin(mThresh),bkgHistDY.FindBin(upperBound)-1,err)
	result["bkgDY"] = val
	result["bkgDYStats"] = 1.+err/val

	if "electron" in channel:
			result["bkgDYScale"] =[ abs(bkgHistDYScaleDown.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"]) , abs(bkgHistDYScaleUp.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"])]
			result["bkgDYPU"] =[ abs(bkgHistDYPUUp.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"]) , abs(bkgHistDYPUDown.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"])]
	if "muon" in channel:

		result["bkgDYScale"] =[ abs(bkgHistDYScaleDown.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"]) , 1.]
		if abs(bkgHistDYSmear.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"]) >=1:
			result["bkgDYRes"] = abs(bkgHistDYSmear.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"])
		else:
			result["bkgDYRes"] = 1./abs(bkgHistDYSmear.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"])
		if abs(bkgHistDYID.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"]) >=1:
			result["bkgDYID"] = abs(bkgHistDYID.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"])
		else:
			result["bkgDYID"] = 1./abs(bkgHistDYID.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"])
	result["bkgDYPDF"] = 1.057378696848 # from Markus's results

	val = bkgHistOther.IntegralAndError(bkgHistOther.FindBin(mThresh),bkgHistOther.FindBin(upperBound)-1,err)
	result["bkgOther"] = val
	if val > 0 and err > 0:
	...
	result["bkgOther"] = abs(bkgHistOther.Integral(dataHist.FindBin(mThresh),bkgHistOther.FindBin(upperBound)-1))
	
	if "electron" in channel:
			if result["bkgOther"] > 0:
				result["bkgOtherScale"] =[ abs(bkgHistOtherScaleDown.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgOther"]) , abs(bkgHistOtherScaleUp.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgOther"])]
			else:
				result["bkgOtherScale"] = [1.,1.]

			result["bkgOtherPU"] =[ 1+abs(bkgHistOtherPUUp.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"]) , 1+abs(bkgHistOtherPUDown.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"])]
	if "muon" in channel:
		if result["bkgOther"] > 0:
			result["bkgOtherScale"] =[ abs(bkgHistOtherScaleDown.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgOther"]) , 1.]
		else:
			result["bkgOtherScale"] =[1.,1.]
		if result["bkgOther"] > 0:
			if  abs(bkgHistOtherSmear.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgOther"]) >= 1:
				result["bkgOtherRes"] = abs(bkgHistOtherSmear.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgOther"])	 
			else:
				result["bkgOtherRes"] = 1./abs(bkgHistOtherSmear.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgOther"])

			if abs(bkgHistOtherID.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgDY"]) >= 1:
				result["bkgOtherID"] = abs(bkgHistOtherID.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgOther"])
			else:
				result["bkgOtherID"] = 1./abs(bkgHistOtherID.Integral(dataHist.FindBin(mThresh),dataHist.FindBin(upperBound)-1)/result["bkgOther"])
		else:
	...
	result["bkgJets"] = bkgHistJets.Integral(bkgHistJets.FindBin(mThresh),bkgHistJets.FindBin(upperBound)-1)
