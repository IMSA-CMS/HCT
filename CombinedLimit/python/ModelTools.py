...
class ModelBuilderBase():
    ...
    def doObj(self,name,type,X,ignoreExisting=False):
        ...
        if self.options.bin: 
            if type == "Poisson":
                type = "RooPoissonFixed"
            elif type == "PROD":
                type = "RooProdPdfFixed"
                if len(X.split(",")) == 1:
                    temp = self.factory_("%s::%s(%s)" % ("PROD", name, X))
                    return temp
                else:
                    temp = self.factory_("%s::%s(%s)" % (type, name+"Kaushal1", X.split(",")[0]+","+X.split(",")[1]))
                for x in range(2,len(X.split(","))):
                    if x != len(X.split(","))-1:
                        temp = self.factory_("%s::%s(%s)" % (type, name+"Kaushal"+str(x), temp.GetName()+","+X.split(",")[x]))
                    else:
                        temp = self.factory_("%s::%s(%s)" % (type, name, temp.GetName()+","+X.split(",")[x]))  
                        return temp
...
class ModelBuilder(ModelBuilderBase):
    ...
    def doNuisances(self):
		...
		if p!= "constr": nuisVars.add(self.out.var(n))
            ...
            self.out.nuisPdf = ROOT.RooProdPdfFixed("nuisancePdf", "nuisancePdf", nuisPdfs)
